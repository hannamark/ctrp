@Global
Feature: PO F20 Read only Version of Person
Read Only users and Abstractors will view all persons details but not make any changes to the person view

  Scenario:#1 I can search for persons with multiple parameters
  ...
  Scenario:#2 I will get a message if searched with no parameters
 ...
 Scenario: #3 I can search PersonExact Search is checked
    ...
    Scenario: #4 I can search person when Exact Search is unchecked
   Given I know the exact paramater type of the person I want to search for
    And I am logged in to CTRP PO application
    And I have selected the option to search for a person
    When I provide the exact parameter I would like to search
  ...
  Scenario:#5 I cannot Search Results when the Exact Search box is checked and Exact Match is not provided
    Given I am logged in to CTRP PO application
  ...
  Scenario:#6 I can view person details
   Given I am logged in to CTRP PO application
	 When I have selected an organization context Source type
     |CTRP|
     |CTEP|
     
     Then I can view Person details in the selected context source
     And I can view selected perosn associations details in different tabs
