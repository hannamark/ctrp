...
Scenario Outline: As any CTRP User, I can request the creation of a new Person Record in CTRP
...
And the Person status should be Pending and the status date should be the current date and time
Examples:
  |Phone      		|  |Email          	|
...
@PO
Scenario Outline: As a PO Curator, I can request the creation of a new Person Record in CTRP
...
@PO
Scenario Outline: As a PO Curator, I will receive a warning if I create a Person with the same name
Given I am logged in to CTRP PO application
And I know the first name, last name, middle name, with any prefix or suffix, phone or email, and organization affiliation of the Person
And I have complete a Search for Person and not found the Person in CTRP
When I have selected the Add Person function
