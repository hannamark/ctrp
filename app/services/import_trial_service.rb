...
class ImportTrialService
  ...
  def import_params (xml, current_user)
    ...
    nlm_org_for_imported_trial = Organization.new(name:org_name,processing_status:processing_status,service_request_id:service_request.id,source_status_id:SourceStatus.find_by_code_and_source_context_id('ACTIVE',nlm_context.id),source_context_id:nlm_context.id)
