  ...
  def wrapper_authenticate_user
    ...
    if token.blank?
      ...
      if user.is_a?(LocalUser)
        ...
        #current_local_user =  @current_user
        sign_in user, :bypass => true
        current_local_user =  user
        #authenticate_local_user!
        #@current_user = current_local_user
        Rails.logger.info "Completed authenticate_user signing in as local_user @current_user =#{@current_user.inspect}"
      elsif user.is_a?(LdapUser)
        #Rails.logger.info "In authenticate_user signing in as ldap_user"
        #authenticate_ldap_user!
        sign_in user, :bypass => true
        current_ldap_user = @current_user
      else
...
=end
 ...
 # end
  ...
  def current_ctrp_user
    current_local_user || current_ldap_user || get_user
  end
