  ...
  def wrapper_authenticate_user
    ...
    if token.blank?
      ...
      unless @current_user.nil?
        ...
        user = get_user
        Rails.logger.info "\nIn Application Controller, wrapper_authenticate_user, Non-angular login, user from get_user=  #{user.inspect}" unless user.nil?
      end
...
=end
 ...
 # end
  ...
  # The current_ctrp_user method is used by the Rails Admin
  # Every Request from the Rails Admin calls this method before authorization checks are performed
  def current_ctrp_user
=begin
    Rails.logger.debug "current_ctrp_user, Params = #{params.inspect}"
    Rails.logger.debug "current_ctrp_user, request= #{request.inspect}"
    Rails.logger.debug "current_ctrp_user, @current_user= #{@current_user.inspect}"
    Rails.logger.debug "current_ctrp_user, current_local_user= #{current_local_user.inspect}"
    Rails.logger.debug "current_ctrp_user, current_ldap_user= #{current_ldap_user.inspect}"
    Rails.logger.debug "current_ctrp_user, request.params= #{request.params.inspect}"
    Rails.logger.debug "current_ctrp_user, rtoken= #{@rtoken}"
    Rails.logger.debug "current_ctrp_user, a= #{get_current_user.inspect}"
    Rails.logger.debug "current_ctrp_user, b= #{get_user.inspect}"

    unless request.params["rtoken"].nil?
      @rtoken = request.params["rtoken"]
    end
    if !@rtoken.nil?
      username = decode_token(request.params["rtoken"] || @rtoken)
